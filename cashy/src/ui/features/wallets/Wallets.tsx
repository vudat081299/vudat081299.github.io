...
import { formatDigits } from "@/domain/money";
...
function WalletEditor({
  ...
  function save() {
    ...
    if (editing) updateWallet(editing.id, { name: n, kind, openingBalance, colorHex: color, icon });
    else addWallet({ name: n, kind, openingBalance, colorHex: color, icon });
    onClose();
  }
        ...
        <div className="wb-cluster" style={{ gap: 12, alignItems: "flex-end" }}>
          ...
          <div className="wb-field" style={{ flex: 1, minWidth: 0 }}>
            <label className="wb-label" htmlFor="wallet-opening">
              Opening balance <span className="wb-label__opt">(đ · − = you owe)</span>
            </label>
            <input
              id="wallet-opening"
              className="wb-input"
...
export function Wallets() {
  ...
  // Active wallets first (by order), archived muted at the end.
  ...
  const activeCount = wallets.filter((w) => !w.archived).length;
  ...
  return (
    <div className="wb-stack wb-stack--loose">
      ...
      <div className="wb-card">
        <div className="wb-card__body" style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          ...
          <span style={{ fontSize: 12, color: "var(--wb-fg-muted)" }}>
            across {activeCount} active {activeCount === 1 ? "wallet" : "wallets"}
          </span>
        </div>
      </div>
      ...
      {wallets.length === 0 ? (
        ...
        >
          {ordered.map((w) => (
            <WalletCard key={w.id} wallet={w} balance={balances.get(w.id) ?? w.openingBalance} onEdit={openEdit} />
          ))}
        </div>
      )}
