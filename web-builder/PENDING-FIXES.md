...
| 5 | `.wb-shell__side` có thể nổi lên như một thẻ trôi nổi, lộ nội dung chính phía sau nó, sau một cú cuộn nhanh/mạnh (bắt trên `data-science-roadmap`, Chrome/Mac, kèm ảnh chụp 2026-08-16) | Nguyên nhân: `.wb-shell__side` từng đặt cả `position: sticky` lẫn `overflow-y: auto` trên CÙNG một phần tử — Chromium có thể tính sai kích thước/vị trí của nó sau cuộn nhanh. Sửa trong kit: tách hai vai ra hai phần tử — `.wb-shell__side` (ngoài) chỉ sticky + `overflow: hidden`; `.wb-sidenav` (trong, con trực tiếp có sẵn) nhận `overflow-y: auto` + padding. Không cần sửa HTML nào — mọi trang đã lồng đúng cấu trúc này sẵn. Cũng đã vá song song trong `web-builder-skill@2991d09` (repo nguồn của skill) |
