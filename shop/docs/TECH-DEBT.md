...
| **`shop/docs/` đang được publish công khai** — `04-NEGOTIATION.md` (kịch bản đàm phán với một người có thật, gồm mục "dấu hiệu nên rút") trả HTTP 200 trên GitHub Pages | Review cuối phiên: `curl` thẳng vào URL đoán được | Loại trừ `shop/docs` và `shop/*.md` khỏi rsync của `deploy.yml`, **cộng một cổng** bắt lỗi nếu hai dòng loại trừ đó biến mất. `shop/pitch/` cố ý vẫn công khai — trang đó viết cho chị ấy. **ĐẢO LẠI 21/09/2026:** chủ repo muốn đọc bộ tài liệu này trên web và coi nó là kế hoạch chứ không phải bí mật; hai dòng loại trừ đã gỡ, cổng đổi sang bắt chiều ngược lại (đỏ nếu chúng quay lại). |
