...
**Tổng:** 84 bài · 11 chặng · 8 tuần · 14 ngày fast track · 106.1 giờ (fast 75.8 giờ)
...
|---|---|---|---|---|---|---|---|
| `s-how` | Trang này dùng thế nào | bắt buộc | 10/0/0 | ✓ | 1 |  | 1556–1659 |
| `s-intro` | ★ Data Science là gì? — và bộ từ vựng tối thiểu | bắt buộc | 30/10/0 | ✓ | 1 |  | 1660–1779 |
| `s-pipeline` | Một product AI thật gồm 10 bước | bắt buộc | 35/0/0 | ✓ | 1 |  | 1780–1833 |
| `s-families` | ★ Bản đồ các họ bài toán Data Science | bắt buộc | 35/10/0 | ✓ | 1 |  | 1834–1879 |
| `s-lookup` | Tra ngược: “vấn đề này thuộc bước nào?” | bắt buộc | 20/0/0 | ✓ | 1 |  | 1880–1934 |
| `s-plan8w` | ★ Lộ trình chuẩn 6–8 tuần | bắt buộc | 20/0/5 | ✓ | 1 |  | 1935–1986 |
| `s-plan14` | Fast track 14 ngày (bản rút gọn) | bắt buộc | 15/0/5 | ✓ | 1 |  | 1987–2040 |
...
| id | bài | ưu tiên | r/x/d | F | T | A | dòng |
|---|---|---|---|---|---|---|---|
| `t-env` | ★ Chỗ làm việc: cloud-first, local là dự phòng | bắt buộc | 25/35/30 | ✓ | 1 |  | 2045–2131 |
| `t-online` | ★ Làm cả dự án trên trình duyệt (máy yếu) | bắt buộc | 25/20/20 | ✓ | 1 |  | 2132–2245 |
| `t-colab` | Google Colab — làm được gì, khi nào dùng | bắt buộc | 20/25/0 | ✓ | 1 |  | 2246–2379 |
| `t-ai` | ★ Làm việc với AI coding assistant | bắt buộc | 30/30/15 | ✓ | 1 |  | 2380–2483 |
| `t-pandas` | pandas — 15 thao tác chiếm 90% việc | bắt buộc | 50/100/20 | ✓ | 1 |  | 2484–2580 |
| `t-sql` | SQL — lấy dữ liệu ra khỏi cơ sở dữ liệu | bắt buộc | 25/35/15 | ✓ | 1 |  | 2581–2706 |
| `t-numpy` | numpy — vector hoá & mảng | bắt buộc | 25/35/10 | ✓ | 1 |  | 2707–2751 |
| `t-sklearn` | scikit-learn — 4 khái niệm là đủ | bắt buộc | 45/75/20 | ✓ | 1 |  | 2752–2809 |
| `t-stack` | ★ Mỗi bước dùng tool gì, và gõ gì | bắt buộc | 60/0/0 | ✓ | 1 |  | 2810–2960 |
...
- **`t-sklearn`** — Bốn khái niệm sklearn: estimator, transformer, Pipeline, ColumnTransformer.
  - dẫn tới: Bài sau nói ở mỗi bước trong 10 bước thì mở công cụ nào và gõ gì.
  - mở nhánh phụ: `standardize`, `x-pipeline-bugs`, `x-sklearn-imports`, `cmp-gbdt`
- **`t-stack`** — Sổ tay theo bước: mở gì, gõ gì, xong có gì trong tay, bẫy nằm ở đâu.
...
| id | bài | ưu tiên | r/x/d | F | T | A | dòng |
|---|---|---|---|---|---|---|---|
| `m-vector` | Vector & ma trận — dữ liệu trong máy | bắt buộc | 55/20/10 | ✓ | 2 |  | 2965–2987 |
| `m-deriv` | Đạo hàm & gradient — “học” nghĩa là gì | bắt buộc | 55/20/10 | ✓ | 2 |  | 2988–3020 |
| `m-prob` | Xác suất: phân phối, kỳ vọng, phương sai | bắt buộc | 55/20/10 | ✓ | 2 |  | 3021–3058 |
| `m-bayes` | Bayes & likelihood | nên biết | 45/15/0 |  | 2 |  | 3059–3084 |
| `m-infer` | Thống kê suy diễn — thứ luận văn cần | bắt buộc | 60/30/10 | ✓ | 2 |  | 3085–3128 |
...
| id | bài | ưu tiên | r/x/d | F | T | A | dòng |
|---|---|---|---|---|---|---|---|
| `d-framing` | Đóng khung bài toán — bước hay bị bỏ | bắt buộc | 30/0/30 | ✓ | 2 | 3 | 3133–3175 |
| `d-eda` | EDA — nhìn dữ liệu đúng cách | bắt buộc | 35/50/45 | ✓ | 2 | 3 | 3176–3265 |
| `d-clean` | Thiếu, lệch, ngoại lai — xử lý ra sao | bắt buộc | 30/25/20 | ✓ | 2 |  | 3266–3329 |
| `d-leak` | Data leakage — lỗi giết chết luận văn | bắt buộc | 45/25/20 | ✓ | 2 | 2 | 3330–3399 |
| `d-split` | Chia tập: random, theo thời gian, theo nhóm | bắt buộc | 35/25/30 | ✓ | 2 | 3 | 3400–3462 |

- **`d-framing`** — Một trang đề bài: dự đoán gì, tại thời điểm nào, sai thì tốn bao nhiêu tiền.
  - dẫn tới: Có tờ đề bài rồi, bài sau mới biết phải nhìn cái gì khi mở dữ liệu ra.
...
| id | bài | ưu tiên | r/x/d | F | T | A | dòng |
|---|---|---|---|---|---|---|---|
| `f-what` | FE khi làm thật là gõ cái gì, ở đâu | bắt buộc | 30/5/10 | ✓ | 3 |  | 3467–3519 |
| `f-numeric` | Biến số: scale, log, clip, binning | bắt buộc | 25/15/20 | ✓ | 3 |  | 3520–3571 |
| `f-cat` | Biến hạng mục: one-hot, target, hashing | bắt buộc | 35/20/20 | ✓ | 3 |  | 3572–3640 |
| `f-cyclic` | ★ Biến chu kỳ: 23h và 0h liền nhau | bắt buộc | 30/15/30 | ✓ | 3 |  | 3641–3750 |
| `f-time` | Biến theo thời gian: lag, rolling, velocity | bắt buộc | 35/15/40 | ✓ | 3 |  | 3751–3835 |
| `f-text` | Chữ & embedding: TF-IDF → vector | nên biết | 40/20/0 |  | 3 |  | 3836–3870 |
| `f-pipeline` | ★ Ráp tất cả: Pipeline + ColumnTransformer | bắt buộc | 30/20/40 | ✓ | 3 | 3 | 3871–3969 |
| `f-select` | Chọn feature & đo tầm quan trọng | nên biết | 30/15/0 |  | 3 |  | 3970–4012 |
| `f-store` | Feature store — khi nào thực sự cần | định vị | 30/0/0 |  | 3 |  | 4013–4035 |
...
- **`f-cyclic`** — Mã hoá sin/cos, gõ được ở cả bốn mức từ notebook tới Pipeline.
  ...
  - mở nhánh phụ: `sincos`, `x-cyclic-more`
...
| id | bài | ưu tiên | r/x/d | F | T | A | dòng |
|---|---|---|---|---|---|---|---|
| `ml-map` | Bản đồ mô hình: bài nào dùng gì | bắt buộc | 45/0/0 | ✓ | 3 |  | 4040–4086 |
| `ml-linear` | Hồi quy tuyến tính & logistic | bắt buộc | 35/20/20 | ✓ | 3 | 3 | 4087–4138 |
| `ml-loss` | Hàm mất mát + gradient descent | bắt buộc | 55/25/10 | ✓ | 4 |  | 4139–4189 |
| `ml-trees` | Cây → Random Forest → Boosting | bắt buộc | 45/35/25 | ✓ | 4 |  | 4190–4264 |
| `ml-overfit` | Overfitting, bias–variance, regularization | bắt buộc | 40/20/15 | ✓ | 4 |  | 4265–4310 |
| `ml-cv` | Cross-validation cho đúng | bắt buộc | 30/10/20 | ✓ | 4 |  | 4311–4354 |
| `ml-imb` | ★ Dữ liệu lệch: 0,17% gian lận thì làm gì | bắt buộc | 30/15/30 | ✓ | 4 |  | 4355–4430 |
| `ml-metrics` | ★ Đo lường: PR-AUC, ngưỡng, và tiền | bắt buộc | 35/20/35 | ✓ | 4 | 3 | 4431–4503 |
| `ml-tune` | Tuning: grid, random, Optuna | nên biết | 25/20/0 |  | 4 |  | 4504–4559 |
| `ml-shap` | Giải thích mô hình: SHAP | nên biết | 30/15/15 |  | 4 |  | 4560–4604 |
| `ml-unsup` | Không nhãn: cụm & phát hiện bất thường | nên biết | 45/15/0 |  | 4 |  | 4605–4639 |
...
- **`ml-linear`** — Baseline hồi quy logistic chạy được, và cách đọc hệ số để viết vào luận văn.
  - dẫn tới: Bài sau nói mô hình đó thực sự được huấn luyện bằng cách nào.
...
- **`ml-imb`** — Thứ tự ưu tiên xử lý mất cân bằng, và bảng thí nghiệm mẫu đưa thẳng vào luận văn.
  - dẫn tới: Bài sau là quyết định dễ làm hỏng cả luận văn nhất.
  - mở nhánh phụ: `auc`
- **`ml-metrics`** — PR-AUC làm chỉ số chính, và một ngưỡng chọn theo chi phí chứ không theo cảm tính.
  - dẫn tới: Có chỉ số đúng rồi, bài sau mới tối ưu siêu tham số mà không tự lừa mình.
  - mở nhánh phụ: `auc`
...
| id | bài | ưu tiên | r/x/d | F | T | A | dòng |
|---|---|---|---|---|---|---|---|
| `dl-nn` | Từ logistic regression → mạng nơ-ron | bắt buộc | 50/15/10 | ✓ | 5 |  | 4644–4694 |
| `dl-backprop` | Backpropagation — thực chất là gì | bắt buộc | 60/15/0 | ✓ | 5 |  | 4695–4729 |
| `dl-train` | Huấn luyện: epoch, batch, lr, early stop | bắt buộc | 50/15/10 | ✓ | 5 |  | 4730–4785 |
| `dl-embed` | Embedding — biến ID thành vector | bắt buộc | 40/10/10 | ✓ | 5 |  | 4786–4828 |
| `dl-cnn-rnn` | CNN & RNN/LSTM — định vị và khi nào còn dùng | nên biết | 55/5/0 |  | 5 |  | 4829–4864 |
| `dl-attn` | ★ Attention — giải từ số 0 | bắt buộc | 90/20/10 | ✓ | 5 |  | 4865–4911 |
| `dl-tf` | ★ Transformer — ráp thành kiến trúc | bắt buộc | 70/10/10 | ✓ | 5 |  | 4912–4961 |
| `dl-llm` | LLM, fine-tune, RAG — dùng sao cho đúng | nên biết | 65/10/0 |  | 5 |  | 4962–5002 |
| `dl-tab` | Deep learning cho dữ liệu bảng: nên không? | nên biết | 40/5/0 |  | 5 |  | 5003–5033 |
...
## 7 · Các họ bài toán khác — chuyển giao kiến thức
...
| id | bài | ưu tiên | r/x/d | F | T | A | dòng |
|---|---|---|---|---|---|---|---|
| `q-regress` | Hồi quy: dự đoán một con số | bắt buộc | 45/20/10 |  | 6 |  | 5042–5107 |
| `q-forecast` | ★ Chuỗi thời gian & forecasting | bắt buộc | 70/50/30 |  | 6 |  | 5108–5218 |
| `q-multi` | Multiclass & multilabel | nên biết | 40/15/5 |  | 6 |  | 5219–5262 |
| `q-nlp` | NLP: TF-IDF → embedding → Transformer | nên biết | 50/30/10 |  | 6 |  | 5263–5319 |
| `q-cv` | Computer vision — định vị & mini-lab | nên biết | 35/25/0 |  | 6 |  | 5320–5373 |
| `q-rec` | Gợi ý & xếp hạng (recommendation) | nên biết | 45/25/5 |  | 6 |  | 5374–5421 |
| `q-causal` | ★ A/B testing & tư duy nhân quả | bắt buộc | 55/25/10 |  | 6 |  | 5422–5489 |
| `q-analytics` | Analytics: SQL, KPI, cohort, funnel | nên biết | 45/25/5 |  | 6 |  | 5490–5544 |
| `q-mini` | ★ Mini-project chuyển giao — làm thật 2/6 | bắt buộc | 45/15/240 |  | 6 | 3 | 5545–5631 |
...
- **`q-mini`** — Hai mini-project chuyển giao làm tới cùng (mỗi cái một notebook + bảng "giữ gì / đổi gì"), và bốn đề phác thảo để tra sau.
  - dẫn tới: Chặng sau quay lại dự án chính và biến nó thành sản phẩm, bắt đầu từ bản vẽ.

## 8 · Làm ra product thật (fraud detection)
...
| id | bài | ưu tiên | r/x/d | F | T | A | dòng |
|---|---|---|---|---|---|---|---|
| `pr-arch` | Kiến trúc hệ thống chống gian lận | bắt buộc | 30/5/25 | ✓ | 7 |  | 5636–5697 |
| `pr-data` | Dữ liệu: lấy ở đâu, dùng bộ nào | bắt buộc | 25/20/30 | ✓ | 7 |  | 5698–6036 |
| `pr-code` | ★ Code end-to-end: repo tới model đã lưu | bắt buộc | 40/50/60 | ✓ | 7 | 3 | 6037–6644 |
| `pr-eval` | ★ Đánh giá, chọn ngưỡng, và bốn test | bắt buộc | 35/30/45 | ✓ | 7 | 4 | 6645–7471 |
| `pr-serve` | Đóng gói: FastAPI + Docker | bắt buộc | 35/40/45 | ✓ | 7 | 3 | 7472–7892 |
| `pr-monitor` | Giám sát: drift, feedback, retrain | bắt buộc | 35/20/25 | ✓ | 7 | 2 | 7893–7954 |
| `pr-mlops` | MLOps tối thiểu: MLflow, DVC, CI | nên biết | 30/25/20 |  | 7 |  | 7955–8008 |
| `pr-cost` | Quy ra tiền: ngưỡng theo chi phí | nên biết | 25/10/15 |  | 7 | 3 | 8009–8052 |

- **`pr-arch`** — Bản vẽ đường đi của một giao dịch, và năm quyết định thiết kế phải nêu trong luận văn.
  - dẫn tới: Bài sau là dữ liệu thật để chạy bản vẽ đó.
- **`pr-data`** — Bộ dữ liệu đã tải và đã kiểm tra, cùng bộ mô phỏng để chạy thử pipeline.
  - dẫn tới: Bài sau là toàn bộ mã nguồn dự án, từ file rỗng tới mô hình đã lưu.
  - mở nhánh phụ: `cmp-storage`
...
- **`pr-eval`** — Ngưỡng chọn theo chi phí, khoảng tin cậy bootstrap, hình PR cho luận văn, và bốn test chặn lỗi im lặng.
  - dẫn tới: Bài sau biến file .joblib đó thành một dịch vụ mà hệ thống khác gọi được.
  - mở nhánh phụ: `ci`, `auc`, `cmp-tune`
...
- **`pr-cost`** — Ma trận chi phí, và phân tích độ nhạy biến điểm yếu thành điểm mạnh.
  - dẫn tới: Sản phẩm xong. Chặng sau là phần viết, bắt đầu từ chọn đề tài.
...
| id | bài | ưu tiên | r/x/d | F | T | A | dòng |
|---|---|---|---|---|---|---|---|
| `th-topic` | Đề tài phải có “delta” — tìm khoảng trống | bắt buộc | 30/5/40 | ✓ | 8 | 2 | 8057–8115 |
| `th-read` | Đọc paper 3 lượt | bắt buộc | 25/35/30 | ✓ | 8 | 3 | 8116–8165 |
| `th-repro` | Tái lập một paper — bước bắt buộc | bắt buộc | 25/30/30 | ✓ | 8 | 2 | 8166–8222 |
| `th-design` | Thiết kế thí nghiệm: baseline, ablation | bắt buộc | 35/20/35 | ✓ | 8 | 2 | 8223–8283 |
| `th-stats` | Kết quả có ý nghĩa thống kê không | bắt buộc | 30/25/30 | ✓ | 8 | 2 | 8284–8337 |
| `th-write` | Viết: IMRaD và câu nào nằm ở đâu | bắt buộc | 40/5/45 | ✓ | 8 | 3 | 8338–8406 |
| `th-tools` | Công cụ: Overleaf, Zotero, W&B | nên biết | 25/20/10 |  | 8 |  | 8407–8455 |
| `th-defense` | ★ Bảo vệ: slide, demo, 24 câu hội đồng hỏi | bắt buộc | 30/5/45 | ✓ | 8 | 3 | 8456–8607 |
...
| id | bài | ưu tiên | r/x/d | F | T | A | dòng |
|---|---|---|---|---|---|---|---|
| `r-roadmapsh` | roadmap.sh dịch ra: giữ gì, bỏ gì | nên biết | 45/0/0 |  | 8 |  | 8612–8698 |
| `r-mistakes` | 20 lỗi người mới hay mắc | bắt buộc | 30/0/0 | ✓ | 8 |  | 8699–8745 |
| `r-glossary` | Từ điển thuật ngữ | nên biết | 45/0/0 |  | 8 |  | 8746–8822 |
| `r-books` | Sách / khoá / dataset đáng dùng | nên biết | 30/0/0 |  | 8 |  | 8823–8866 |
...
|---|---|---|---|
| `dot` | Vector và tích vô hướng — “giống nhau” đo bằng gì | 648–665 | `t-numpy`, `m-vector`, `dl-attn`, `dl-llm` |
| `matmul` | Ma trận — vì sao mọi lỗi đều là lỗi shape | 666–677 | `t-numpy`, `m-vector` |
| `deriv` | Đạo hàm & gradient — “học” thực chất là gì | 678–691 | `m-deriv` |
| `chain` | Quy tắc dây chuyền — cỗ máy đằng sau backpropagation | 692–702 | `dl-nn`, `dl-backprop` |
| `gdupdate` | Công thức cập nhật — dòng lệnh mà máy chạy hàng triệu lần | 703–718 | `s-lookup`, `m-deriv`, `ml-loss`, `dl-nn`, `dl-train` |
| `expvar` | Kỳ vọng, phương sai, độ lệch chuẩn | 719–735 | `s-how`, `m-prob`, `d-clean` |
| `normal` | Phân phối chuẩn và z-score | 736–748 | `m-prob` |
| `bayes` | Định lý Bayes — vì sao 99% chính xác vẫn có thể vô dụng | 749–764 | `s-intro`, `m-bayes` |
| `sigmoid` | Sigmoid & log-odds — biến điểm số thành xác suất | 765–778 | `ml-linear` |
| `logloss` | Cross-entropy (log loss) — sai số cho bài toán phân loại | 779–793 | `ml-loss` |
| `mse` | MSE, RMSE, MAE — sai số cho bài toán dự đoán số | 794–807 | `ml-linear`, `ml-loss` |
| `entropy` | Entropy & Gini — cây quyết định chọn câu hỏi thế nào | 808–821 | `ml-trees` |
| `reg` | Regularization L1/L2 — buộc mô hình khiêm tốn lại | 822–836 | `ml-overfit`, `dl-train` |
| `sincos` | Mã hoá chu kỳ bằng sin/cos — toán đằng sau nó | 837–861 | `s-lookup`, `f-cyclic` |
| `softmax` | Softmax — biến điểm số thành phân phối xác suất | 862–874 | `ml-loss`, `dl-nn`, `dl-attn`, `q-multi` |
| `attention` | Scaled dot-product attention — công thức đầy đủ | 875–897 | `dl-attn` |
| `posenc` | Positional encoding — công thức sin/cos trong Transformer gốc | 898–915 | `dl-tf` |
| `pvalue` | p-value — điều nó nói và điều nó KHÔNG nói | 916–937 | `m-infer`, `q-causal`, `th-stats` |
| `ci` | Khoảng tin cậy & bootstrap — cách báo cáo kết quả cho tử tế | 938–965 | `m-infer`, `q-causal`, `pr-eval`, `th-stats` |
| `auc` | ROC-AUC và PR-AUC — hai đường cong, hai câu chuyện | 966–982 | `ml-imb`, `ml-metrics`, `pr-eval` |
| `standardize` | Standardize hay Min-Max — chọn cái nào | 983–997 | `t-sklearn`, `m-vector`, `f-numeric` |
| `logtransform` | Vì sao lấy log của số tiền | 998–1013 | `s-lookup`, `m-prob`, `f-numeric`, `q-regress` |
| `mcnemar` | McNemar — so hai mô hình trên cùng tập test | 1014–1032 | `m-infer`, `th-stats` |

## Nhánh phụ — ngăn bên phải (`data-aside`)

| khoá | tiêu đề | dòng | bài nào mở |
|---|---|---|---|
| `cmp-run` | Chỗ viết và chạy code — và chỗ mượn GPU | 1041–1071 | `s-how`, `s-lookup`, `t-env`, `t-online`, `t-colab`, `t-stack` |
| `cmp-storage` | Để vài trăm MB dữ liệu ở đâu | 1072–1087 | `s-lookup`, `t-env`, `t-online`, `t-colab`, `t-stack`, `pr-data` |
| `cmp-query` | Lấy dữ liệu ra: pandas, DuckDB, SQLite hay máy chủ thật | 1088–1102 | `s-lookup`, `t-sql`, `t-stack` |
| `cmp-eda` | Nhìn dữ liệu: tự viết hay để công cụ sinh báo cáo | 1103–1117 | `s-lookup`, `t-stack`, `d-eda` |
| `cmp-dataframe` | Xử lý bảng: pandas, polars hay đẩy sang SQL | 1118–1132 | `s-lookup`, `t-pandas`, `t-stack`, `d-clean` |
| `cmp-gbdt` | Boosting: LightGBM, XGBoost, CatBoost hay sklearn | 1133–1147 | `s-lookup`, `t-sklearn`, `t-stack`, `ml-trees`, `pr-code` |
| `cmp-tune` | Tinh chỉnh siêu tham số: grid, random hay Optuna | 1148–1161 | `s-lookup`, `t-stack`, `ml-tune`, `pr-eval` |
| `cmp-track` | Ghi lại thí nghiệm: file CSV, MLflow hay W&amp;B | 1162–1177 | `s-lookup`, `t-stack`, `pr-monitor`, `pr-mlops` |
| `cmp-explain` | Giải thích mô hình: SHAP, permutation, LIME | 1178–1194 | `s-lookup`, `t-stack`, `ml-shap` |
| `cmp-serve` | Đóng gói và chạy thật: khung nào, đặt ở đâu | 1195–1237 | `s-lookup`, `t-online`, `t-stack`, `pr-serve` |
| `cmp-figure` | Hình cho luận văn: biểu đồ và sơ đồ kiến trúc | 1238–1262 | `s-lookup`, `t-stack`, `th-write`, `th-tools` |
| `cmp-write` | Viết luận văn: soạn thảo và quản lý trích dẫn | 1263–1286 | `s-lookup`, `t-online`, `t-ai`, `t-stack`, `th-tools` |
| `x-notneeded` | Những thứ chưa cần cho dự án này — và mốc để quay lại | 1290–1305 | `t-stack` |
| `x-la-skip` | Đại số tuyến tính: phần bỏ qua được | 1306–1311 | `m-vector` |
| `x-tree-gradient` | Cây quyết định và XGBoost có dùng gradient không? | 1312–1322 | `m-deriv` |
| `x-memorize` | Mô hình có phải “học thuộc” không? | 1323–1328 | `ml-map` |
| `x-tree-learn` | Cây và boosting thì “học” thế nào | 1329–1334 | `ml-loss` |
| `x-tf-variants` | Ba biến thể Transformer, và paper gốc | 1335–1347 | `dl-tf` |
| `x-tab-dl-zoo` | Các kiến trúc deep learning cho dữ liệu bảng | 1348–1359 | `dl-tab` |
| `x-attn-kinds` | Ba loại attention bạn sẽ gặp tên | 1360–1369 | `dl-attn` |
| `x-llm-vocab` | Từ vựng LLM để không bị hớ | 1370–1382 | `dl-llm` |
| `x-mlops-skip` | MLOps: những thứ chưa cần ở quy mô này | 1383–1395 | `pr-mlops` |
| `x-cyclic-more` | Còn cột nào có tính chu kỳ nữa | 1396–1408 | `f-cyclic` |
| `x-pipeline-bugs` | Hai lỗi Pipeline hay gặp và cách sửa | 1409–1428 | `t-sklearn`, `f-pipeline`, `pr-code` |
| `x-sklearn-imports` | Bản đồ import: cần gì thì lấy từ đâu | 1429–1441 | `t-sklearn` |
| `x-split-other` | Hai cách chia khác: phân tầng và theo nhóm | 1442–1454 | `d-split` |

## Lịch 8 tuần

| tuần | deliverable | mốc | bài |
|---|---|---|---|
| 1 | Repo GitHub (<code>data/</code> trong <code>.gitignore</code>); một notebook Colab clone repo và đọc được một CSV; <code>requirements.txt</code> đã ghim phiên bản. <i>(problem-statement.md để tuần 2, khi học đóng khung.)</i> |  | `s-how` `s-intro` `s-pipeline` `s-families` `s-lookup` `s-plan8w` `s-plan14` `t-env` `t-online` `t-colab` `t-ai` `t-pandas` `t-sql` `t-numpy` `t-sklearn` `t-stack` |
| 2 | <code>docs/problem-statement.md</code> (dự đoán gì · khi nào · sai tốn bao nhiêu); <code>notebooks/01-eda.ipynb</code> + 4 hình trong <code>reports/figures/</code>; <code>src/split.py</code> kèm <code>assert_split_ok</code>; danh sách cột nghi rò rỉ kèm lý do |  | `m-vector` `m-deriv` `m-prob` `m-bayes` `m-infer` `d-framing` `d-eda` `d-clean` `d-leak` `d-split` |
| 3 | <code>src/features.py</code> + <code>src/history.py</code>; <code>src/pipeline.py</code> (Pipeline + ColumnTransformer) với một test một dòng; <code>notebooks/02-baseline.ipynb</code> chạy hết và in PR-AUC validation từ logistic; bảng thí nghiệm có dòng 1. <i>(<code>python -m src.train</code> chỉ được yêu cầu ở tuần 7, sau bài pr-code.)</i> | Mốc 1 | `f-what` `f-numeric` `f-cat` `f-cyclic` `f-time` `f-text` `f-pipeline` `f-select` `f-store` `ml-map` `ml-linear` |
| 4 | Bảng so sánh baseline vs mô hình chính (LightGBM) trên cùng validation: PR-AUC + trung bình±độ lệch chuẩn của 5 seed; một hình SHAP; một ngưỡng chọn theo chi phí. <i>(Khoảng tin cậy cluster bootstrap và kiểm định ghép cặp làm ở tuần 7–8 khi đã học đúng loại bootstrap.)</i> |  | `ml-loss` `ml-trees` `ml-overfit` `ml-cv` `ml-imb` `ml-metrics` `ml-tune` `ml-shap` `ml-unsup` |
...
| 6 | Hai mini-project làm thật (mỗi cái một notebook + bảng “giữ gì / đổi gì” so với fraud); bốn đề còn lại chỉ cần bảng giữ/đổi ở mức phác thảo | Mốc 2 | `q-regress` `q-forecast` `q-multi` `q-nlp` `q-cv` `q-rec` `q-causal` `q-analytics` `q-mini` |
| 7 | <code>python -m src.train</code> bản đầy đủ + <code>models/fraud-*.joblib</code> + metadata + ngưỡng; <code>src/evaluate.py</code> (cluster bootstrap CI + <code>paired_ci</code>); API FastAPI + <code>Dockerfile</code>; hàm PSI; CI chạy <code>pytest</code> |  | `pr-arch` `pr-data` `pr-code` `pr-eval` `pr-serve` `pr-monitor` `pr-mlops` `pr-cost` |
| 8 | <code>research-question.md</code>; reading-note skeleton + lượt 1–2 cho một paper; <b>reproduction report skeleton</b>; bảng ablation + registry thí nghiệm; kiểm định ghép cặp; <b>thesis skeleton</b> (mapping artifact→chương); defense-plan skeleton. <i>(Lượt đọc 3, viết luận văn hoàn chỉnh, tái lập một paper thật, làm slide/rehearsal và bảo vệ nằm NGOÀI 8 tuần — cần nhiều tuần/tháng và phản hồi mentor.)</i> | Mốc 3 | `th-topic` `th-read` `th-repro` `th-design` `th-stats` `th-write` `th-tools` `th-defense` `r-roadmapsh` `r-mistakes` `r-glossary` `r-books` |
...
| 2 | Xây dựng dữ liệu đáng tin | EDA notebook + data/README (datacard) + danh sách rò rỉ | `d-eda` `d-clean` `d-leak` `pr-data` |
